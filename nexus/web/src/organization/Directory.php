...
class Catalogue {
	...
	public static function getDirectoryEntries($groupId, $orgUid, $inputString, $filters) {
		...
		if (strlen($terms) > 0) {
			...
			while ($counter < 3 && isset($searchTerms[$counter])) {
				...
				$cursor = self::freeSearch($searchTerms[$counter], $orgUid);
		...
		// We get here if there are no free search terms. So, run filters against the entire network
			...
			$cursor = pgDb::getOrgsByNetwork($orgUid);
	...
	private static function freeSearch($term, $orgUid) {
		...
		$query = "
			...
			return pgDb::psExecute($query, array($term, $orgUid));			
