/*
                ...
                detectJava_winLoaded.js v0.5.4
 ...
 USAGE:
  ...
  5) Insert this script AFTER the PluginDetect script. It does not matter if this script
     is before or after the output <div> and the plugindetect <div>, because it waits for the
     web page to load before plugin detection begins.
...
function WL(){
     ...
     docWrite("[window.onload event has fired successfully.");
     docWrite("Detection occurs after window.onload event has fired.]");
     docWrite("");
...
// Call WL after the window has loaded
if ($.onWindowLoaded) $.onWindowLoaded(WL)
else window.onload = WL;
