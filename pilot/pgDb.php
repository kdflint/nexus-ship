...
class pgDb {
	...
	public static function userEmailExists($email) {
		$query = "select exists (select true from public.user where email = '$email')";
		return pgDb::execute($query);
	}
	
	public static function countUserEmail($email) {
		$query = "select id from public.user where email = '$email'";
		return pg_num_rows(pgDb::execute($query));
	}
	
	public static function userOrgRelationExists($orgId, $email) {
		$query = "select exists (select true from user_organization where organization_fk = '$orgId' and user_fk in (select id from public.user where email = '$email'))";
		return pgDb::execute($query);
	}
	
	public static function insertOrganization($name, $ein) {
			$query = "insert into organization (name, ein) values ('$name', '$ein') returning id";
			return pgDb::execute($query);		
	}
	...
	public static function insertActiveUser($uuid, $email) {
			$query = "insert into public.user (uuid, email, status, create_dttm, activate_dttm) values ('$uuid', '$email', 'active', now(), now()) returning id";
			return pgDb::execute($query);		
	}
